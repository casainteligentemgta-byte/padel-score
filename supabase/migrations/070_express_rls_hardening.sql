...
-- Service role (API, Telegram, cron) → sin cambios (bypass RLS)
-- =============================================================================
...
COMMENT ON FUNCTION public.guard_express_matches_client_write() IS
  'Bloquea escrituras Express peligrosas desde anon/authenticated (QR, session_id, marcador en standby).';
...
-- Sin políticas para anon/authenticated → solo service_role + trigger SECURITY DEFINER.
