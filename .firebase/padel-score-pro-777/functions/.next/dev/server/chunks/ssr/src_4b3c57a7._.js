...
function CourtCard({ venueName, courtKey, displayCourtNum, title, libraryVideos, libraryImages, videoRows, imageRows, tiraList, linkedTiraIds, videoCambioMinutos, imagenCambioMinutos, tiraCambioMinutos, imagenLoop, imagenPausaSeg, onSaveVideoPlaylist, onSaveImagePlaylist, onSaveTiraPlaylist, lastSeenIso, isSaving }) {
   ...
   */ const previewIframeSrc = venueName.trim().length > 0 ? `/display/court/${displayCourtNum}?complex=${encodeURIComponent(venueName)}${minimalMode ? '&minimal=1' : ''}` : `/display/court/${displayCourtNum}${minimalMode ? '?minimal=1' : ''}`;
...
//# sourceMappingURL=src_4b3c57a7._.js.map