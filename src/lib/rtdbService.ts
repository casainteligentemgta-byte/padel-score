...
function requireRtdb() {
    if (!rtdb) throw new Error('Realtime Database no configurada. Añade NEXT_PUBLIC_FIREBASE_DATABASE_URL en .env.local (ej: https://tu-proyecto-default-rtdb.firebaseio.com)');
    return rtdb;
}
...
export async function activarCancha(
    ...
    const canchaRef = ref(requireRtdb(), `canchas/${canchaId}`);
    await set(canchaRef, {
...
export async function actualizarMarcador(canchaId: string, marcador: Record<string, any>) {
    const marcadorRef = ref(requireRtdb(), `canchas/${canchaId}/marcador`);
...
export async function setModoPuntos(
    ...
    await update(ref(requireRtdb(), `canchas/${canchaId}/marcador`), {
        modo_puntos: modo,
        puntos: { local: '0', visitante: '0' },
        ultimo_update: Date.now(),
    });
...
export async function getCanchaOnce(canchaId: string) {
    const snap = await get(ref(requireRtdb(), `canchas/${canchaId}`));
    return snap.val();
}
...
export async function setModoPublicidad(modo: ModoPublicidad) {
    await update(ref(requireRtdb(), 'publicidad_master'), { modo, ultimo_update: Date.now() });
}

/** Activa/desactiva el carrusel */
export async function toggleCarrusel(activo: boolean) {
    await update(ref(requireRtdb(), 'publicidad_master'), {
...
export async function setImagenFija(url: string) {
    await update(ref(requireRtdb(), 'publicidad_master/fija'), { url });
    await update(ref(requireRtdb(), 'publicidad_master'), { ultimo_update: Date.now() });
}

/** Programa un banner con rango horario (epoch ms) */
export async function setProgramada(url: string, inicioMs: number, finMs: number) {
    await update(ref(requireRtdb(), 'publicidad_master/programada'), {
        activa: true,
        url,
...
export async function setImagenCarrusel(imgId: string, url: string, orden: number, activa = true) {
    await set(ref(requireRtdb(), `publicidad_master/imagenes/${imgId}`), { url, orden, activa });
    await update(ref(requireRtdb(), 'publicidad_master'), { ultimo_update: Date.now() });
}

/** Elimina una imagen del carrusel */
export async function deleteImagenCarrusel(imgId: string) {
    await set(ref(requireRtdb(), `publicidad_master/imagenes/${imgId}`), null);
}
...
export async function setRTDBUserRole(
    ...
    await set(ref(requireRtdb(), `usuarios_roles/${uid}`), {
        rol,
        nombre,
        email,
...
export async function initPublicidadMaster() {
    const snap = await get(ref(requireRtdb(), 'publicidad_master'));
    if (!snap.exists()) {
        await set(ref(requireRtdb(), 'publicidad_master'), {
            modo: 'fija',
...
export async function setTickerConfig(activo: boolean, texto: string, velocidad_seg: number) {
    await update(ref(requireRtdb(), 'publicidad_master/ticker'), {
        activo,
        texto,
        velocidad_seg,
    });
...
export async function setCronometroTipo(tipo: CronometroTipo) {
    await update(ref(requireRtdb(), 'publicidad_master'), { cronometro_tipo: tipo, ultimo_update: Date.now() });
}
...
export async function setRelojTipo(tipo: RelojTipo) {
    await update(ref(requireRtdb(), 'publicidad_master'), { reloj_tipo: tipo, reloj_ocasion: tipo, ultimo_update: Date.now() });
}
...
export async function setRelojModelo(modeloId: string, data: { nombre: string; items: { url: string; orden: number }[]; rotacion_seg: number }) {
    await set(ref(requireRtdb(), `publicidad_master/reloj_modelos/${modeloId}`), data);
    await update(ref(requireRtdb(), 'publicidad_master'), { ultimo_update: Date.now() });
}

/** Selecciona qué modelo de reloj está activo */
export async function setRelojModeloActivo(modeloId: string | null) {
    await update(ref(requireRtdb(), 'publicidad_master'), { reloj_modelo_activo: modeloId ?? null, ultimo_update: Date.now() });
}
...
export async function setLogoEvento(items: { id: string; url: string; orden: number }[], rotacion_seg: number) {
    ...
    await set(ref(requireRtdb(), 'publicidad_master/logo_evento'), { items: obj, rotacion_seg });
    await update(ref(requireRtdb(), 'publicidad_master'), { ultimo_update: Date.now() });
}
...
export async function setLogosPatrocinantes(items: { id: string; url: string; orden: number }[], rotacion_seg: number) {
    const obj: Record<string, { url: string; orden: number }> = {};
    items.forEach(i => { obj[i.id] = { url: i.url, orden: i.orden }; });
    await set(ref(requireRtdb(), 'publicidad_master/logos_patrocinantes'), { items: obj, rotacion_seg });
    await update(ref(requireRtdb(), 'publicidad_master'), { ultimo_update: Date.now() });
}
...
export async function setVideoEsquina(items: { id: string; url: string; orden: number }[], rotacion_seg: number) {
    const obj: Record<string, { url: string; orden: number }> = {};
    items.forEach(i => { obj[i.id] = { url: i.url, orden: i.orden }; });
    await set(ref(requireRtdb(), 'publicidad_master/video_esquina'), { items: obj, rotacion_seg });
    await update(ref(requireRtdb(), 'publicidad_master'), { ultimo_update: Date.now() });
}
...
export async function setTickerTipoYAnimaciones(tipo: 'texto' | 'animado', animaciones: { id: string; url: string; orden: number }[]) {
    ...
    await update(ref(requireRtdb(), 'publicidad_master/ticker'), { tipo_animacion: tipo, animaciones: obj });
    await update(ref(requireRtdb(), 'publicidad_master'), { ultimo_update: Date.now() });
}
...
export async function setAnimacionMarcador(animId: string, data: { nombre: string; url: string } | null) {
    if (data === null) {
        await set(ref(requireRtdb(), `publicidad_master/animaciones_marcador/${animId}`), null);
    } else {
        await set(ref(requireRtdb(), `publicidad_master/animaciones_marcador/${animId}`), data);
    }
    await update(ref(requireRtdb(), 'publicidad_master'), { ultimo_update: Date.now() });
}
...
export async function dispararAnimacionMarcador(canchaId: string, animId: string | null) {
    const pathRef = ref(requireRtdb(), `canchas/${canchaId}/animacion_actual`);
    if (animId) {
